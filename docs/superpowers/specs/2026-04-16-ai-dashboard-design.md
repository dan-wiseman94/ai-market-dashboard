...
11. **M11 — Second brain**: `apps.thesis` — `Thesis` (ticker/direction/conviction/entry/target/invalidation/horizon, status `open|closed_win|closed_loss|closed_scratch|invalidated`), `PostMortem` (one per horizon in `THESIS_POSTMORTEM_HORIZONS=[7,30,90]`, unique on `thesis+horizon_days`), `DecisionJournalEntry` (acted/passed/watching/hedged per thread); endpoints `/api/theses/` (CRUD + `close` + `run-postmortem` actions) and `/api/journal/`. Post-mortems: deterministic `objective_verdict` from `apps.market.returns.forward_return_pct` (DEADZONE 1%) closes the loop with no AI key; best-effort Claude structured output (`PostMortemReport`) posted as an assistant Message into a per-thesis review thread — gracefully degrades to `report={}` on non-claude provider/missing key/cap hit/any error, never raises. Idempotent `scheduled→running` atomic claim prevents double-billing on beat+run-now overlap. `thesis.run_due_postmortems` beat task (300s). `apps.thesis` added to the explicit Celery autodiscover list. `AgentPreset` model in `apps.profiles` + `/api/presets/`; four builtins seeded by data migration (`earnings-prep`, `devils-advocate`, `pre-trade-bias-check`, `triage-pass`); builtin flag is read-only. Shared `apps/market/returns.py` (`nearest_bar_close`, `forward_return_pct`, `price_path_summary`) extracted from analytics leaderboard. Frontend: `ThesesPage`/`ThesisDetailPage` at `/theses|/theses/:id`, `g j` shortcut, `go-theses` Cmd-K command, "Close & journal" + "Promote to thesis" on `ThreadDetailPage`, preset dropdown in `SnapshotComposerPage`, preset management in `ProfilesPage`. Plan: `docs/superpowers/plans/2026-05-25-m11-second-brain.md`.
