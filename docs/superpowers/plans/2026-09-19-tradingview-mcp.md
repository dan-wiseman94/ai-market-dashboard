...
- Everything runs in Docker. Backend tests: `docker compose exec web pytest apps/<app>/tests/test_<x>.py -v` (container WORKDIR is `/app/backend`). Frontend tests: `docker compose exec frontend pnpm exec vitest run <path>`.
