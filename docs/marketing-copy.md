...
- Capture a point-in-time snapshot of the market — quotes, OHLC, option chains with positioning analytics (put/call ratios, max pain, IV skew, dealer gamma), sector breadth and rotation, a VIX term structure on every capture, macro and yield-curve data, news, and server-rendered chart PNGs.
