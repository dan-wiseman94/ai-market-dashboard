...
Run `git diff HEAD` (and `git diff --staged`) and check each item the change actually touches.
Report `file:line` + fix for real issues only.
...
- **Providers:** obtained via `get_provider()` / router, never instantiated directly.
- **MOCK_EXTERNAL:** not added to `compose.yaml`, env files, or dev settings (e2e overlay only).
