...
A single-user desktop dashboard that captures point-in-time **stock-market snapshots** (quotes, intraday + daily OHLC, option chains with positioning analytics, positions, breadth + sector rotation, an always-on VIX term structure, macro & yield-curve data, news, fundamentals, rendered charts) and routes them to an AI — **Claude**, **OpenAI**, or a **local OpenAI-compatible endpoint** — for observations framed by a named trading style and a per-snapshot objective. It then closes the loop: record the decision you made, and let a scheduled post-mortem grade it against the tape.
...
- **Snapshots** — capture a point-in-time market picture from opt-in sections: real-time **quotes**, **OHLC** history (a rolling 24-hour intraday window on the primary ticker — newest ~4 h at 1-minute bars — plus **daily history for every watchlist ticker** and a longer-horizon summary off 52 weeks of stored daily bars: 252-session high/low, SMA distances, 5/20/60-day returns), **option chains**, **positions**, **market breadth**, **news**, **company fundamentals** (P/E, EPS, margins, growth, beta, dividend yield, 52-week range via Finnhub), **macro indicators** (FRED series — including **HY/IG credit-spread** OAS — + live CBOE yield indices), **SEC filings** (EDGAR 10-K / 10-Q / 8-K + **Form 4 insider**), **Treasury rates**, **rendered chart images** (headless-Chromium PNGs), the **events** forward calendar (with corporate-action ex-dates), and your free-text **note** as its own section. The default eight sections (quotes / positions / breadth / ohlc / chain / news / events / macro) are pre-checked for every new profile, and every capture also ships an always-on **VIX term structure** section (below). A section that fails is marked `failed` and explicitly flagged in the AI payload — partial captures are fine, never silently dropped.
- **VIX term structure, always on** — every capture, on every path (manual / observer / trigger / briefing), carries $VIX spot plus the front- and second-month **/VX futures** with basis and a contango / backwardation read, built on CFE settlement-date math — plus **VVIX** and the VVIX/VIX ratio. Without Schwab it degrades to spot-only with an explicit note — never silently omitted, and never pruned under token pressure.
- **Breadth + sector rotation** — the breadth section batches the index complex ($SPX / SPY / QQQ, plus /ES and /NQ via a separate fallback-safe quote), the **dollar** (UUP), all **11 SPDR sector ETFs** (rendered as a last / 1d% / 5d% / RS table), and NYSE internals ($ADVN / $DECN / $TICK / $TRIN / $UVOL / $DVOL, gated until they warm up pre-open), then adds the primary ticker's **relative strength vs. $SPX** over 1 / 5 / 20 sessions, a 5-session **sector-rotation** read, **factor-ETF returns + spreads** (momentum−value, small−large, growth−value), and computed breadth stats (% above 20/50-SMA, fresh high/low counts) from stored daily bars.
- **Options positioning analytics** — the chain section renders the two nearest expiries plus an analytics block computed over *every* fetched expiry: **put/call volume and open-interest ratios**, **max pain**, **25-delta IV skew**, the **ATM-IV term structure**, and a **dealer gamma-exposure estimate** (total GEX, the **by-strike gamma walls**, and the zero-gamma flip strike, under a documented dealers-long-calls / short-puts heuristic) — plus top volume/OI strikes per side and an **unusual-activity** block.
- **Fed communications + flow proxy** — a keyless `fed` section pulls the Federal Reserve's public RSS feeds (press releases, speeches, testimony) plus a days-to-next-FOMC line; a `flowlite` section derives a **volume-based proxy** for unusual activity (volume z-scores vs. the 20-day average, put/call-ratio deltas vs. the prior chain) — always labeled a proxy, never presented as real fund-flow data. Both are opt-in.
...
- **Forward calendar** — upcoming **earnings** (per-ticker, BMO/AMC hints + EPS estimates via Finnhub) and curated US **macro** events (FOMC / CPI / NFP / PCE / GDP). Surfaced three ways: the opt-in `events` snapshot section, the `days_to_earnings` trigger leaf, and `GET /api/market/events/?tickers=…&within_days=14`. Refreshed daily by the `market.refresh_events` beat task; Finnhub's economic calendar is a premium endpoint, so on a free key the curated seed is the effective macro source (FOMC dates through 2027; CPI / NFP / PCE / GDP through late 2026 until those calendars publish).
- **Free data sources alongside Schwab** — beyond the Schwab integration, the backend ships free-tier / keyless clients for **Alpaca** (real-time IEX quotes + bars), **Tiingo**, **Twelve Data**, **Polygon** (price history), **Tradier** (delayed option chains), **FRED** (macro + the daily Treasury yield curve), **SEC EDGAR** (filings + Form 4 insider), **Marketaux** (news + per-ticker sentiment), **US Treasury** FiscalData, and the **Federal Reserve**'s own RSS feeds (`fed` section — press releases, speeches, testimony; keyless). Drop a key into **Settings → Connections** and the quotes / OHLC / option-chain / news pipeline transparently falls back to a free provider when Schwab isn't connected — so the whole dashboard runs without a brokerage login. `macro`, `filings`, `treasury`, `fed`, and `flowlite` are opt-in snapshot sections.
...
- **Token budgeting** — the payload is trimmed to a per-model budget (from the model catalog) before it reaches the LLM: watchlist daily history goes first, then the oldest OHLC bars, then whole sections in a fixed order (chain → ohlc → news → fed → flowlite → breadth → quotes → positions; the VIX, macro, and events sections are never pruned). Per-section token counts are recorded for the cost drill-down.
- **Snapshot diff** — `GET /api/snapshots/<id>/diff/?against=<id>` returns a markdown delta between two captures.
- **Expected-move overlay** — the option-chain section feeds the model an options-implied **1σ move** (`atm_iv × √(days/365)`, nearest-expiry) plus a 7 / 30 / 90-day term structure; each `AIPrediction` freezes it at decision time, so the scorecard reports a **beat-the-straddle** rate — how often the real move beat what was priced, and how often it did so in the right direction.
- **Themes / narrative tracker** — group tickers into narratives (AI-capex, GLP-1, …) and read each one's **breadth**, **leadership**, and **relative strength vs. `$SPX`** from split-corrected OHLC. CRUD + health live at `/api/themes/` (`GET /api/themes/<id>/health/`) and it's browsable at `/themes`; coverage degrades honestly below two priced members.
