...
- **Snapshots** — capture a point-in-time market picture from opt-in sections: real-time **quotes**, **OHLC** history (52 weeks of stored daily bars, with a longer-horizon summary — 252-session high/low, SMA distance, 5/20/60-day returns), **option chains** (dealer **GEX by strike** — the top gamma walls — plus top volume/OI strikes and an unusual-activity block), **positions**, **market breadth** (a sector table, the dollar, and cross-asset factor returns alongside the index complex), **news**, **macro indicators** (FRED, including **HY/IG credit-spread** series), **SEC filings** (EDGAR + Form 4 insider), **Treasury rates**, **rendered chart images** (headless-Chromium PNGs), and the **events** forward calendar. A section that fails is marked `failed` and explicitly flagged in the AI payload — partial captures are fine, never silently dropped. The default eight sections (quotes/positions/breadth/ohlc/chain/news/events/macro) are pre-checked for every new profile. An always-on **VIX** section rides along on every capture — spot plus `/VX` futures term structure, **VVIX**, and the VVIX/VIX ratio.
