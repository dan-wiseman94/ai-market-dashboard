...
- **Snapshots** — capture a point-in-time market picture from opt-in sections: real-time **quotes**, **OHLC** history (a rolling 24-hour intraday window on the primary ticker — newest ~4 h at 1-minute bars — plus **daily history for every watchlist ticker**), **option chains**, **positions**, **market breadth**, **news**, **company fundamentals** (P/E, EPS, margins, growth, beta, dividend yield, 52-week range via Finnhub), **macro indicators** (FRED series + live CBOE yield indices), **SEC filings** (EDGAR 10-K / 10-Q / 8-K), **Treasury rates**, **rendered chart images** (headless-Chromium PNGs), the **events** forward calendar, and your free-text **note** as its own section. Every capture also ships an always-on **VIX term structure** section (below). A section that fails is marked `failed` and explicitly flagged in the AI payload — partial captures are fine, never silently dropped.
- **VIX term structure, always on** — every capture, on every path (manual / observer / trigger / briefing), carries $VIX spot plus the front- and second-month **/VX futures** with basis and a contango / backwardation read, built on CFE settlement-date math. Without Schwab it degrades to spot-only with an explicit note — never silently omitted, and never pruned under token pressure.
- **Breadth + sector rotation** — the breadth section batches $SPX / QQQ / $VIX, all **11 SPDR sector ETFs**, and NYSE internals ($ADVN / $DECN / $TICK / $TRIN, gated until they warm up pre-open), then adds the primary ticker's **relative strength vs. $SPX** over 1 / 5 / 20 sessions and a 5-session **sector-rotation** read (leaders and laggards) computed from stored daily bars.
- **Options positioning analytics** — the chain section renders the two nearest expiries plus an analytics block computed over *every* fetched expiry: **put/call volume and open-interest ratios**, **max pain**, **25-delta IV skew**, the **ATM-IV term structure**, and a **dealer gamma-exposure estimate** (total GEX + the zero-gamma flip strike, under a documented dealers-long-calls / short-puts heuristic).
...
- **Forward calendar** — upcoming **earnings** (per-ticker, BMO/AMC hints + EPS estimates via Finnhub) and curated US **macro** events. Surfaced three ways: the opt-in `events` snapshot section, the `days_to_earnings` trigger leaf, and `GET /api/market/events/?tickers=…&within_days=14`. Refreshed daily by the `market.refresh_events` beat task; Finnhub's economic calendar is a premium endpoint, so on a free key the curated seed (FOMC / CPI / NFP dates) is the effective macro source.
- **Free data sources alongside Schwab** — beyond the Schwab integration, the backend ships free-tier / keyless clients for **Alpaca** (real-time IEX quotes + bars), **Tiingo**, **Twelve Data**, **Polygon** (price history), **Tradier** (delayed option chains), **FRED** (macro + the daily Treasury yield curve), **SEC EDGAR** (10-K / 10-Q / 8-K filings), **Marketaux** (news + per-ticker sentiment), and **US Treasury** FiscalData. Drop a key into **Settings → Connections** and the quotes / OHLC / option-chain / news pipeline transparently falls back to a free provider when Schwab isn't connected — so the whole dashboard runs without a brokerage login. `macro`, `filings`, and `treasury` are new opt-in snapshot sections.
...
- **Token budgeting** — the payload is trimmed to a per-model budget (from the model catalog) before it reaches the LLM: watchlist daily history goes first, then the oldest OHLC bars, then whole sections in a fixed order (chain → ohlc → news → breadth → quotes → positions; the VIX, macro, and events sections are never pruned). Per-section token counts are recorded for the cost drill-down.
