...
def _mk_bar(
    ticker: str, ts: datetime, close: float, *, high: float | None = None, low: float | None = None
) -> None:
...
class TestNearestBarClose:
    def test_returns_close_for_exact_timestamp(self, db) -> None:
        _mk_bar("AAPL", BASE, 150.0)
        result = nearest_bar_close("AAPL", _aware(BASE))
        assert result == pytest.approx(150.0)

    def test_returns_most_recent_bar_within_1h_window(self, db) -> None:
    ...
    def test_prefers_most_recent_bar_when_multiple_exist(self, db) -> None:
        _mk_bar("AAPL", BASE - timedelta(hours=2), 140.0)
        _mk_bar("AAPL", BASE, 150.0)
        result = nearest_bar_close("AAPL", _aware(BASE))
        assert result == pytest.approx(150.0)

    def test_returns_none_when_no_bars_for_ticker(self, db) -> None:
        _mk_bar("AAPL", BASE, 150.0)
    ...
    def test_does_not_return_bar_more_than_1h_beyond_at(self, db) -> None:
        # bar is 2h in the future relative to `at` — outside the lte window
        _mk_bar("AAPL", BASE + timedelta(hours=2), 200.0)
        result = nearest_bar_close("AAPL", _aware(BASE))
        assert result is None
...
def test_forward_return_pct_parametrized(db, start_close, end_close, expected) -> None:
    ...
    _mk_bar("AAPL", BASE, start_close)
    _mk_bar("AAPL", BASE + timedelta(hours=24), end_close)
    result = forward_return_pct("AAPL", start, end)
    assert result == pytest.approx(expected, rel=0.01)


def test_forward_return_pct_none_when_no_start_bar(db) -> None:
    start = _aware(BASE)
    end = _aware(BASE + timedelta(hours=24))
...
def test_forward_return_pct_none_when_t0_has_no_bar(db) -> None:
...
def test_forward_return_pct_none_when_start_close_is_zero(db) -> None:
    start = _aware(BASE)
    end = _aware(BASE + timedelta(hours=24))
    _mk_bar("AAPL", BASE, 0.0)
    _mk_bar("AAPL", BASE + timedelta(hours=24), 10.0)
    result = forward_return_pct("AAPL", start, end)
    assert result is None


def test_forward_return_pct_none_for_unknown_ticker(db) -> None:
    start = _aware(BASE)
    end = _aware(BASE + timedelta(hours=24))
    _mk_bar("AAPL", BASE, 100.0)
    _mk_bar("AAPL", BASE + timedelta(hours=24), 110.0)
...
class TestPricePathSummary:
    ...
    def test_single_bar_in_range(self, db) -> None:
        _mk_bar("AAPL", BASE + timedelta(hours=1), 100.0, high=105.0, low=95.0)
        start = _aware(BASE)
    ...
    def test_start_close_and_end_close_correct(self, db) -> None:
        _mk_bar("AAPL", BASE, 100.0)
        _mk_bar("AAPL", BASE + timedelta(hours=24), 115.0)
        start = _aware(BASE)
        end = _aware(BASE + timedelta(hours=24))
        result = price_path_summary("AAPL", start, end)
    ...
    def test_return_pct_matches_forward_return_pct(self, db) -> None:
        _mk_bar("AAPL", BASE, 100.0)
        _mk_bar("AAPL", BASE + timedelta(hours=24), 115.0)
        start = _aware(BASE)
        end = _aware(BASE + timedelta(hours=24))
    ...
    def test_bars_count_includes_only_range(self, db) -> None:
        # bars at BASE-1h (outside), BASE+1h (inside), BASE+2h (inside), BASE+25h (outside)
        _mk_bar("AAPL", BASE - timedelta(hours=1), 99.0)
        _mk_bar("AAPL", BASE + timedelta(hours=1), 100.0)
        _mk_bar("AAPL", BASE + timedelta(hours=2), 101.0)
        _mk_bar("AAPL", BASE + timedelta(hours=25), 110.0)
        start = _aware(BASE)
        end = _aware(BASE + timedelta(hours=24))
        result = price_path_summary("AAPL", start, end)
        assert result["bars"] == 2

    def test_max_high_min_low_are_floats_not_decimal(self, db) -> None:
        _mk_bar("AAPL", BASE + timedelta(hours=1), 100.0, high=110.0, low=90.0)
        start = _aware(BASE)
        end = _aware(BASE + timedelta(hours=2))
        result = price_path_summary("AAPL", start, end)
    ...
    def test_unknown_ticker_returns_empty(self, db) -> None:
        _mk_bar("AAPL", BASE + timedelta(hours=1), 100.0)
        start = _aware(BASE)
        end = _aware(BASE + timedelta(hours=2))
    ...
    def test_bar_in_grace_window_affects_end_close_not_bars_or_highs(self, db) -> None:
        ...
        _mk_bar("AAPL", BASE, 100.0, high=100.0, low=100.0)  # at start, inside range
        grace_bar_ts = BASE + timedelta(hours=5, minutes=30)  # inside (end, end+1h]
        _mk_bar("AAPL", grace_bar_ts, 200.0, high=250.0, low=50.0)  # outside strict range
