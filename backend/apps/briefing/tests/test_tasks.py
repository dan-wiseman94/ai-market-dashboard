...
def test_skips_before_send_at():
    ...
    cfg.save()
    with patch(
        "apps.briefing.tasks._now_local", return_value=datetime(2026, 5, 28, 7, 0, tzinfo=UTC)
    ):
...
def test_fires_when_due():
    ...
    with (
        patch(
            "apps.briefing.tasks._now_local", return_value=datetime(2026, 5, 28, 9, 0, tzinfo=UTC)
        ),
