...
def test_theses_section_computes_distances():
    ...
    with patch(
        "apps.briefing.services.assemble.fetch_quotes", return_value={"NVDA": {"last": 100.0}}
    ):
...
def test_assemble_combines_sections_and_captures_snapshot():
    ...
    with (
        ...
        patch(
            "apps.briefing.services.assemble.upcoming_events",
        ...
        patch(
            "apps.briefing.services.assemble._capture_market",
...
def test_assemble_never_raises_when_a_section_fails():
    ...
    with (
        patch(
            "apps.briefing.services.assemble._watchlist_union", side_effect=RuntimeError("db down")
        ),
        patch("apps.briefing.services.assemble._capture_market", return_value=(None, {})),
        patch("apps.briefing.services.assemble._theses_section", side_effect=RuntimeError("boom")),
        patch("apps.briefing.services.assemble.upcoming_events", side_effect=RuntimeError("boom")),
        patch(
            "apps.briefing.services.assemble._triggers_section", side_effect=RuntimeError("boom")
        ),
