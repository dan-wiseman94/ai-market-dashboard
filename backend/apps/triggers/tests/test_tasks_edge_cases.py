...
def test_redis_factory_builds_a_client_without_connecting():
    from apps.triggers.tasks import _redis
...
def test_evaluator_exception_disables_the_trigger():
    ...
    with (
        patch("apps.triggers.tasks.metrics.build_snapshot", return_value={}),
        patch("apps.triggers.tasks.cooldown_blocks", return_value=False),
        patch("apps.triggers.tasks.evaluator.evaluate", side_effect=ValueError("bad leaf")),
    ):
        evaluate_triggers()
