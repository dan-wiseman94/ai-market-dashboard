...
``apps.coverage.services.revise.run_structured`` (the name bound in the service
...
PATCH_TARGET = "apps.coverage.services.revise.run_structured"
...
def test_cost_cap_exceeded_skips_without_calling_ai(profile, provider_cfg, snapshot):
    with (
        patch(
            "apps.coverage.services.revise.check_daily_cap",
