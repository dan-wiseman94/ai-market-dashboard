...
# (the config.celery default — correct only for the idempotent claim/lock-carrying
# tasks; the other billing tasks opt out the same way) — otherwise a worker killed
# mid-stream redelivers and re-bills + re-streams a duplicate. A lost run surfaces
# as an incomplete stream the user can retry, never a silent double-charge.
