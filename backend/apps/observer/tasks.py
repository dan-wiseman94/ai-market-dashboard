...
# fire_observer tasks execute in parallel for the same schedule on the concurrency>1
...
# inside fire_observer (the plain path delegates to the already-at-most-once
...
# The registered name stays "observer.run_observer" — existing PeriodicTask rows
# and sync_periodic_task point at that string; changing it silently stops every
# cron schedule from firing.
@shared_task(name="observer.run_observer", acks_late=False, reject_on_worker_lost=False)
def fire_observer_task(schedule_id: int) -> int | None:
    ...
    try:
        ...
        return fire_observer(schedule_id)
    if not acquired:
