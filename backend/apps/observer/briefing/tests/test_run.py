...
from apps.observer.briefing.services import run as R
...
def test_run_briefing_scheduled_is_idempotent_per_day(cfg):
    with (
        ...
        patch("apps.observer.briefing.services.run.run_ai_on_message.delay"),
        patch("apps.observer.briefing.services.run.notify"),
        patch("apps.observer.briefing.services.run._local_today", return_value=date(2026, 5, 28)),
    ):
