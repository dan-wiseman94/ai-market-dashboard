...
SYNCHRONOUSLY inside ``fire_observer`` (only the plain path delegates to the
...
def test_fire_observer_is_at_most_once() -> None:
    assert fire_observer_task.acks_late is False
    assert fire_observer_task.reject_on_worker_lost is False
