...
function useDefaultCommands(onShowHelp: () => void): Command[] {
  ...
  return useMemo(
    () => [
      ...
      { id: "go-features", label: "Open Features settings",
        keywords: "feature flags toggles switches capabilities enable disable turn on off env",
        run: () => nav("/settings/features") },
      ...
      { id: "go-scorecard", label: "Go to Scorecard", keywords: "calibration brier conviction hit rate trust",
        run: () => nav("/scorecard") },
      { id: "go-mirror", label: "Go to The Mirror", keywords: "self calibration decisions passed winners conviction reliability",
        run: () => nav("/mirror") },
      { id: "go-regime", label: "Go to Regime", keywords: "market regime volatility risk-on risk-off breadth",
        run: () => nav("/regime") },
      { id: "go-book", label: "Go to Book", keywords: "risk exposure x-ray positions concentration",
        run: () => nav("/book") },
      { id: "go-warroom", label: "Go to War Room", keywords: "debate personas bull bear courtroom multi-agent",
        run: () => nav("/warroom") },
      { id: "go-desk", label: "Go to Desk", keywords: "anomaly sweep agentic findings",
        run: () => nav("/desk") },
      { id: "go-predictions", label: "Go to Predictions",
        keywords: "prediction ledger directional calls forecast hit rate resolved horizon",
        run: () => nav("/predictions") },
      { id: "go-lessons", label: "Go to Lessons",
        keywords: "lesson coach distilled post-mortem rules pinned muted mistakes",
        run: () => nav("/lessons") },
      { id: "go-coverage", label: "Go to Coverage",
        keywords: "coverage house view ticker stance conviction standing note revisions",
        run: () => nav("/coverage") },
